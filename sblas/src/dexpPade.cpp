...
void dexpPadeApprox::_dexp_pade_low()
{
    ...
    memcpy(VecMatE[k + 4]->v, VecMatE[k + 2]->v, sizeof(REAL_TYPE) * d * d);                                                                     // E[k+4] <- LU
    cblas_daxpy(d * d, -1.0, VecMatE[k + 3]->v, 1, VecMatE[k + 4]->v, 1);                                                                        // E[k+4] <- LU - LV
    memcpy(VecMatE[k + 5]->v, VecMatE[k + 2]->v, sizeof(REAL_TYPE) * d * d);                                                                     // E[k+5] <- LU
    cblas_daxpy(d * d, 1.0, VecMatE[k + 3]->v, 1, VecMatE[k + 5]->v, 1);                                                                         // E[k+5] <- LU + LV
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, d, d, d, 1.0, VecMatE[k + 4]->v, d, VecMatM[k + 5]->v, d, 1.0, VecMatE[k + 5]->v, d); // Es[12] <- LU + LV + (LU - LV) * R
    PLU.Solve(VecMatE[k + 5]->v, d, d);
}
