...
int main(int argc, char *argv[])
{
    ...
    ArrVec<REAL_TYPE> TimeLoops(loops * 20);
    ...
    for (INTE_TYPE outerloop = 0; outerloop < 2; outerloop++)
    {
        ...
        {
            dim = dim_beg + dim_ind;
            if ((dim <= 400) && (outerloop == 1))
                cur_loop = loops * 20;
            else if ((dim <= 600) && (outerloop == 1))
                cur_loop = loops * 10;
            else if ((dim <= 800) && (outerloop == 1))
                cur_loop = loops * 4;
            else
                cur_loop = outerloop == 0 ? 1 : loops;
            if (outerloop)
            ...
            if (paras["-task"] == "forward")
            {
                ...
                TIME_AND_AVG(tlv, cur_loop, Record(dim_ind, record_ind++), {}, { dexp_cmpx.Assign(ssf); dexp_cmpx.Parameter_Skew(); }, {});
