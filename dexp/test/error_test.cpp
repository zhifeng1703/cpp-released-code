...
int main(int argc, char *argv[])
{
    ...
    // INTE_TYPE testD = 5;
    // auto TestA = SkewSymmMat(testD);
    // TestA.init_low_vec();
    // for (int i = 0; i < TestA.lsize; i++)
    // 	TestA.lv[i] = i + 1;
    // TestA.vec2mat();
    // TestA.printf();
    // auto TestSsf = SkewSchurFactor(testD);
    // TestSsf.Factor_SkewSymm(TestA.v, testD);
    // TestSsf.Explict_Vector();
    // auto TestCmpx = dexpSkewEigenPara(testD);
    // TestCmpx.Assign(TestSsf);
    // auto TestVal = ColMat<CMPX_TYPE>(testD, testD);
    // auto TestWork = ColMat<CMPX_TYPE>(testD, testD);
    // auto TestS = ColMat<REAL_TYPE>(testD, testD);
    // memset(TestVal.v, 0, sizeof(CMPX_TYPE) * testD * testD);
    // for (int i = 0; i < testD; i++)
    // 	assignCMPX(TestVal.v[testD * i + i], TestCmpx.EigVal.v[i]);
    // CMPX_TYPE complex0, complex1;
    // assignCMPX(complex0, 0.0, 0.0);
    // assignCMPX(complex1, 1.0, 0.0);
    // cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, testD, testD, testD, &complex1, TestCmpx.EigVec.v, testD, TestVal.v, testD, &complex0, TestWork.v, testD); // work = R * Y;
    // cblas_zgemm(CblasColMajor, CblasNoTrans, CblasConjTrans, testD, testD, testD, &complex1, TestWork.v, testD, TestCmpx.EigVec.v, testD, &complex0, TestVal.v, testD);
    // // cblas_zgemm(CblasColMajor, CblasNoTrans, CblasConjTrans, testD, testD, testD, &complex1, TestCmpx.EigVec.v, testD, TestCmpx.EigVec.v, testD, &complex0, TestVal.v, testD);
    // for (int i = 0; i < testD * testD; i++)
    // 	TestS.v[i] = TestVal.v[i].real;
    // TestS.printf();
