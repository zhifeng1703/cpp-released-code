function [elapsed_forward, elapsed_inverse] = test_dexp(dims, scale)
...
%   See also: sblas_dexp, pade_dexp, dk_dexp.
    ...
    for dim = dims
        ...
        S = randn(dim); S = 0.5*(S - S') * scale;
        X = randn(dim); X = 0.5*(X - X');
    ...
    % ============================================================
    % PLOT 1 — FORWARD differential timing
    % ============================================================
    figure;
    plot(dims, elapsed_forward(:,1), '-o', 'LineWidth',1.8,...
        'DisplayName','sBLAS Dexp (fwd)');
    hold on;
    plot(dims, elapsed_forward(:,2), '-x', 'LineWidth',1.8,...
        'DisplayName','Padé p=3 (fwd)');
    plot(dims, elapsed_forward(:,3), '-s', 'LineWidth',1.8,...
        'DisplayName','Padé p=13 (fwd)');
    plot(dims, elapsed_forward(:,4), '-^', 'LineWidth',1.8,...
        'DisplayName','Daletskii-Krein');
    hold off;
    ...
    % ============================================================
    % PLOT 2 — INVERSE differential timing
    % ============================================================
    figure;
    plot(dims, elapsed_inverse(:,1), '-o', 'LineWidth',1.8,...
        'DisplayName','sBLAS Dexp (inv)');
    hold on;
    plot(dims, elapsed_inverse(:,2), '-x', 'LineWidth',1.8,...
        'DisplayName','Padé p=1 (inv)');
    plot(dims, elapsed_inverse(:,3), '-s', 'LineWidth',1.8,...
        'DisplayName','Padé p=7 (inv)');
    plot(dims, elapsed_inverse(:,4), '-^', 'LineWidth',1.8,...
        'DisplayName','Daletskii-Krein');
    hold off;
...
function elapsed = elap_forward(n, S, X, opt)
    ...
    % 1. sBLAS forward
    tic;
    Yss = sblas_dexp(S, X, 'fwd');
    elapsed(1) = toc;

    % 2. Padé p=3
    tic;
    Yp3 = pade_dexp(S, X, 'fwd', 3);
    elapsed(2) = toc;

    % 3. Padé p=13
    tic;
    Yp13 = pade_dexp(S, X, 'fwd', 13);
    elapsed(3) = toc;

    % 4. Finite-difference surrogate
    tic;
        Ydk = dk_dexp(S, X, 'fwd');
    elapsed(4) = toc;
...
function elapsed = elap_inverse(n, S, X, opt)
    ...
    % First obtain forward reference (for inverse input)

    % 1. sBLAS inverse
    tic;
    Yi = sblas_dexp(S, X, 'inv');
    elapsed(1) = toc;

    % 2. Padé inverse p=1
    tic;
    Yi1 = pade_dexp(S, X, 'inv', 1);
    elapsed(2) = toc;

    % 3. Padé inverse p=7
    tic;
    Yi7 = pade_dexp(S, X, 'inv', 7);
    elapsed(3) = toc;

    % 4. FD surrogate for inverse
    tic;
    Yidk = dk_dexp(S, X, 'inv');
    elapsed(4) = toc;
