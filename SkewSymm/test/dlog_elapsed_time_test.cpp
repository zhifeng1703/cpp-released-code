...
int main(int argc, char *argv[])
{
	...
	for (INTE_TYPE outerloop = 0; outerloop < 2; outerloop++)
	{
		for (INTE_TYPE dim_ind = 0; dim_ind < dim_end - dim_beg + 1; dim_ind++)
		{
			...
			auto dlog_pade = dlogPadeApproximant(dim);

			SchurAngular_SkewSymm_Spectral(A_saf_real, MatA);
			A_saf_real.compute_SpecOrthMat(MatQ);
			my_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, dim, dim, dim, 1.0, MatQ.v, dim, MatM.v, dim, 0.0, MatDelta.v, dim);
			...
			// |************************ Preprocessing ************************|
			// +---------------------------------------------------------------+
			Record_Time[2][dim_ind][1] = 0;
			for (INTE_TYPE loop = 0; loop < loops; loop++)
			{
				beg = std::chrono::steady_clock::now();

				dlog_pade.setupSAF(&A_saf_real); // Computation being timed.
			...
			for (INTE_TYPE loop = 0; loop < loops; loop++)
			{
				...
				dlog_pade.dlog(MatN, MatDelta, MatQ);
